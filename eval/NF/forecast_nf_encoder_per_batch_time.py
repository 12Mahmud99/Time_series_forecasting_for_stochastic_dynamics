...
def build_model(device, cfg, n_past, n_future, encoder_type="gru"):
    ...
    model_variant = cfg.get("model_variant", "ar")
...
def run_forecast_batched(model, positions, context_length, prediction_length,
    ...
    total_time = 0.0
    per_item_times = []   # list of averaged time per trajectory for each batch
    ...
    for b in tqdm(range(n_batches), desc="Forecasting"):
        ...
        with torch.no_grad():
            ...
            if isinstance(out, tuple):
                ...
                samp = out
            batch_elapsed = timelib.time() - start_t

        total_time += batch_elapsed

        # Distribute batch time evenly across trajectories in this batch
        avg_time_per_traj = batch_elapsed / B
        per_item_times.extend([avg_time_per_traj] * B)

        samp_np = samp.cpu().numpy().reshape(B, n_samples, prediction_length)
        samples_out[b_start:b_end] = samp_np.transpose(0, 2, 1)

    # Convert to numpy array for saving
    per_item_times = np.array(per_item_times, dtype=np.float32)

    print(f"Inference time: {total_time:.2f}s  "
          f"({total_time/N*1000:.1f} ms/traj, "
          f"{total_time/(N*n_samples)*1000:.3f} ms/sample)")

    return samples_out, gt_np, ctx_np, total_time, per_item_times
...
def main():
    ...
    samples, ground_truth, contexts, time_elapsed, per_item_times = run_forecast_batched(
        model=model,
        positions=positions,
    ...
    print(f"per_item_times: mean={per_item_times.mean():.3f}s, "
          f"min={per_item_times.min():.3f}s, max={per_item_times.max():.3f}s")
