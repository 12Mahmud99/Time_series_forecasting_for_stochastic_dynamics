#!/usr/bin/env bash
#SBATCH --job-name=traffic_nips_100_100
...
#SBATCH --gres=gpu:1
#SBATCH --mem=24G
#SBATCH --output=logs/uncond_tsdiff/traffic_nips_100_100.%j.out
#SBATCH --error=logs/uncond_tsdiff/traffic_nips_100_100.err
source /packages/apps/mamba/2.0.8/etc/profile.d/conda.sh
conda activate venv310
which python
mkdir -p checkpoints_25_25/tsdiff/traffic_nips
mkdir -p checkpoints_50_50/tsdiff/traffic_nips
mkdir -p checkpoints_100_100/tsdiff/traffic_nips
source /packages/apps/mamba/2.0.8/etc/profile.d/conda.sh
...
python train/TSDiff/train_tsdiff.py \
        ...
        --config configs/tsdiff_train/traffic_nips_100_100.yaml \
        --out_dir checkpoints_100_100/tsdiff/traffic_nips