...
#SBATCH --gres=gpu:1
#SBATCH --mem=24G
#SBATCH --output=logs/tsdiff_cond/tcond5.%j.out
#SBATCH --error=logs/tsdiff_cond/tcond5.err

source /packages/apps/mamba/2.0.8/etc/profile.d/conda.sh
conda activate venv310

mkdir -p checkpoints_25_25/tsdiff_cond/electricity_nips
mkdir -p checkpoints_50_50/tsdiff_cond/electricity_nips
mkdir -p checkpoints_100_100/tsdiff_cond/electricity_nips
...
python train/TSDiff/train_cond_tsdiff.py \
        ...
        --config configs/tsdiff_cond_train/electricity_nips_100_100.yaml \
        --out_dir checkpoints_100_100/tsdiff_cond/electricity_nips