#!/usr/bin/env bash
#SBATCH --job-name=tsdiff_cond_electricity_nips
...
#SBATCH --mem=32G
#SBATCH --output=logs/tsdiff_cond/electricity_nips.%j.out
#SBATCH --error=logs/tsdiff_cond/electricity_nips.err
...
mkdir -p checkpoints_custom/tsdiff_cond/electricity_nips
...
python eval/TSDiff/forecast_tsdiff_cond_nips.py   \
        --config configs/tsdiff_forecast/electricity_cond.yaml  \
        --dataset_path gluonts_datasets/electricity_nips \
        --out results/tsdiff_cond/electricity_nips.npz 