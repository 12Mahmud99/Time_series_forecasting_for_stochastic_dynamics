#!/usr/bin/env bash
#SBATCH --job-name=tsdiff_cond_traffic_nips
...
#SBATCH --mem=32G
#SBATCH --output=logs/tsdiff_cond/tsdiff_cond_traffic_nips.%j.out
#SBATCH --error=logs/tsdiff_cond/tsdiff_cond_traffic_nips.err
...
mkdir -p checkpoints_custom/tsdiff_cond/tsdiff_cond_trafficy_nips
...
python eval/TSDiff/forecast_tsdiff_cond_nips.py   \
        --config configs/tsdiff_forecast/traffic_cond.yaml  \
        --dataset_path gluonts_datasets/traffic_nips \
        --out results/tsdiff_cond/traffic_nips.npz 