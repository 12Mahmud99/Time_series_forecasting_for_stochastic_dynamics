...
#SBATCH --gres=gpu:1
#SBATCH --mem=24G
#SBATCH --output=logs/tsdiff_cond/tcond6.%j.out
#SBATCH --error=logs/tsdiff_cond/tcond6.err

source /packages/apps/mamba/2.0.8/etc/profile.d/conda.sh
conda activate venv310

mkdir -p checkpoints_25_25/tsdiff_cond/traffic_nips
mkdir -p checkpoints_50_50/tsdiff_cond/traffic_nips
mkdir -p checkpoints_100_100/tsdiff_cond/traffic_nips
...
python train/TSDiff/train_cond_tsdiff.py \
        ...
        --config configs/tsdiff_cond_train/traffic_nips_100_100.yaml \
        --out_dir checkpoints_100_100/tsdiff_cond/traffic_nips